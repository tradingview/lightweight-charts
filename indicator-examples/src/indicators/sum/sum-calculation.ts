...
/**
 * Options for sum calculation between two sets of series data.
 *
 ...
 */
export interface SumCalculationOptions<
...
> {
	...
	/**
	 * If true, allows calculation even when dates do not match exactly.
	 */
...
/**
 * Calculates sum indicator values between two data series.
 *
 * For each item in the primary data, finds the closest corresponding item in the secondary data (by time),
 * and add their values. If values are missing or dates are mismatched
 ...
 * @param {SumCalculationOptions<TPrimaryData, TSecondaryData>} options - Options for calculation, such as which property to use and mismatch handling.
 * @returns {(LineData | WhitespaceData)[]} An array of values (or whitespace data) for each time in the primary series.
 *
 * @throws {Error} If a source property is not provided and cannot be inferred from the data.
 *
 * @example
 * const result = calculateSumIndicatorValues(
 *   [{ time: 1, value: 10 }, { time: 2, value: 20 }],
 *   [{ time: 1, value: 8 }, { time: 2, value: 15 }],
 *   { primarySource: 'value', secondarySource: 'value' }
 * );
 */
export function calculateSumIndicatorValues<
	TPrimaryData extends SupportedData,
	TSecondaryData extends SupportedData
>(
	primaryData: TPrimaryData[],
	secondaryData: TSecondaryData[],
	options: SumCalculationOptions<TPrimaryData, TSecondaryData>
	...
	if (!primaryDataSource) {
		throw new Error(
			'Please provide a `primarySource` for the primary data of the sum indicator.'
		);
	}
	...
	if (!secondaryDataSource) {
		throw new Error(
			'Please provide a `secondarySource` for the secondary data of the sum indicator.'
		);
	}
