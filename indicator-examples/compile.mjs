...
function findIndicatorFiles(folderPath, recursive) {
	...
	pathNames.forEach(pathName => {
		...
		if (recursive && stats.isDirectory() && pathName === basename(fullPath)) {
			const innerFiles = findIndicatorFiles(fullPath, false);
...
const indicatorsFolder = resolve(currentDir, 'src', 'indicators');
const indicatorFiles = findIndicatorFiles(indicatorsFolder, true);

const filesToBuild = indicatorFiles.map(([filepath, exportName]) => {
	return {
		filepath,
		exportName,
...
const compile = async () => {
	...
	console.log('Bundling the indicators...');
