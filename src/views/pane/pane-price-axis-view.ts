...
class PanePriceAxisViewRenderer implements IPaneRenderer {
	...
	public draw(ctx: CanvasRenderingContext2D, devicePixelRation: number): void {
		if (this._rendererOptions === null || this._priceAxisViewRenderer === null) {
			return;
		}

		this._priceAxisViewRenderer.draw(ctx, this._rendererOptions, this._textWidthCache, this._width, this._align, devicePixelRation);
	}
}
