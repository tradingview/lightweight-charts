...
import { BrandMarkWidget } from './brand-mark-widget';
...
export class PaneWidget implements IDestroyable, MouseEventHandlers {
    ...
    private _brandMarkWidget: BrandMarkWidget | null = null;
	...
	public setState(pane: Pane | null): void {
		...
		if (this._chart.paneWidgets().indexOf(this) === this._chart.paneWidgets().length - 1) {
                    this._brandMarkWidget = this._brandMarkWidget ?? new BrandMarkWidget(this._paneCell, this._chart);
                    this._brandMarkWidget.update();
		} else {
                    this._brandMarkWidget?.removeElement();
                    this._brandMarkWidget = null;
		}
	}
