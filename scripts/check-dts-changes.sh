...
# This script switches the working tree between two revisions. Everything is
# defined in functions that are parsed before the first `git checkout`, so
# the running script never depends on reading more of this file after the
# tree has changed underneath it.
...
function main {
  ...
  local OUT_DIR=$1
  local BASE_GIT_REV=$2
  local COMPARE_GIT_REV=$3

  mkdir -p $OUT_DIR

  local BASE_DTS_FILE=$OUT_DIR/base.d.ts
  local COMPARE_DTS_FILE=$OUT_DIR/compare.d.ts
  local DIFF_FILE=$OUT_DIR/changes.diff
