...
async function beforeInteractions(container) {
	...
	for (const factory of [false, true]) {
		...
		const series = factory ? LwcPlugin.createStackedAreaSeries(chart, options) : chart.addCustomSeries(new LwcPlugin.StackedAreaSeries(), options);
		const data = [0, 2, 4, 6, 8].map(index => ({ time: time(index), values: [10, 5] }));
		series.setData(data);
		...
		const painted = () => {
			...
			const y = series.priceToCoordinate(5);
		...
		if (factory) {
			...
			series.update({ time: time(4), values: [10, 5] }, true);
			await frames();
