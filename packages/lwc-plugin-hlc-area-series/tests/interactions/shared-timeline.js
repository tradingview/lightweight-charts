...
async function beforeInteractions(container) {
	...
	for (const factory of [false, true]) {
		...
		const series = factory ? LwcPlugin.createHLCAreaSeries(chart, options) : chart.addCustomSeries(new LwcPlugin.HLCAreaSeries(), options);
		const data = [0, 2, 4, 6, 8].map(index => ({ time: time(index), high: 20, low: 10, close: 15 }));
		series.setData(data);
		...
		const painted = () => {
			...
			const y = series.priceToCoordinate(12);
		...
		if (factory) {
			...
			series.update({ time: time(4), high: 20, low: 10, close: 15 }, true);
			await frames();
