...
async function beforeInteractions(container) {
	...
	const series = LwcPlugin.createBrushableAreaSeries(chart, {
		priceLineVisible: false, lastValueVisible: false,
		topColor: '#00a000', bottomColor: '#00a000',
	});
	const build = hole => Array.from({ length: 6000 }, (_unused, i) =>
		i % 7 === 6 || (hole !== null && i >= hole && i < hole + 512)
			? { time: time(i) }
			: { time: time(i), value: 30 });
	...
	const sample = () => {
		...
		// Ten pixels under the line: the base price sits below the visible range.
		const y = Math.floor((series.priceToCoordinate(30) + 10) * ratio);
