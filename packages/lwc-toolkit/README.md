...
| `custom-series/options-aware-series` | `createOptionsAwareSeries`, `OptionsAwareSeries` — bind price-value builders to current options and refresh stored values when scaling options change |
...
| `custom-series/visible-bars` | `forEachVisibleBar`, `mapVisibleBars`, `extendRange`, `visibleSegments`, `barCoordinate`, `getConflationFactor` — iterate the visible bars only, reach the pane edge, split at whitespace gaps |
