...
- A `--workspace` mode for scaffolding official plugin packages inside the
  ...
  `CHANGELOG.md` and `NOTICE`, and adds a catalog preview page.
...
- New wizard questions for the plugin description, licence, minimum supported
  Lightweight Charts™ version and catalog tags. The answers populate the
  `lwcPlugin` block in `package.json` that the plugin catalog reads.
...
- The generated `package.json` follows the conventions of the Lightweight
  Charts™ plugin catalog, so a published plugin can be listed there.
