...
or later by invoking the `moveToPane(index)` method on the series instance.
...
`layout.panes` chart options, and `separatorHoverColor` to change the color when the mouse is over the separator.
...
Additionaly we can control each pane via the `PaneApi`. It has few methods to get some data about the pane,
