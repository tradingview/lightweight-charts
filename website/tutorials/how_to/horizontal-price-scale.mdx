...
The `IHorzScaleBehavior` interface in the Lightweight Charts™ library allows
developers to customize the behavior of the horizontal scale. By default, this
scale uses [time](/docs/api#time) values, but you can override it to use any
other type of horizontal scale items, such as price values. The most typical use
case is the creation of Options charts.
...
## Example: Implementing a Price-Based Horizontal Scale with Precision
...
### Using the Custom Horizontal Scale Behavior

To use this custom horizontal scale behavior, instantiate the
