...
This communication will be implemented with [hooks](https://react.dev/reference/react/hooks) and [composition](https://reactjs.org/docs/composition-vs-inheritance.html).

One drawback of hooks, such as `useEffect`, in a parent–child setup is that hooks run in a bottom-up order during instantiation, but in a top-down order during cleanup.
...
In the example above, `ChildComponent` (_Series_) is created first, even though it depends on a `ParentComponent` (_Chart_).
To make this possible, you should use hooks with [refs](https://react.dev/learn/manipulating-the-dom-with-refs), which allow a parent to share internal references or properties directly with its children.

Considering this, the component structure and usage remain the same, but under the hood it works as follows:
