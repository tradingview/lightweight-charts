...
`demoPath` names the demo page's source; nothing builds or deploys it yet. The demo that is deployed today is the gallery copy of the example at `/plugin-examples/plugins/<slug>/example/`, which imports the published package. Deciding whether the catalogue embeds that page or a build of the package demo is part of the pages task.
