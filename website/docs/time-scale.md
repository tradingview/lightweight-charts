...
The argument is [Time](/api/#time) that needs to be converted into a coordinate.
...
Converts `x` coordinate to [Time](/api/#time).
The argument is the `x` coordinate that needs to be converted into a time.

Returns [Time](/api/#time) of a bar that is located on the passed coordinate or `null` if there are no bars on that coordinate.
...
- an object with properties `from` and `to` of type [Time](/api/#time), that are the first and last visible time points, respectively
