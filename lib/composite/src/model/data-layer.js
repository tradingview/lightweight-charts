...
const get_series_plot_row_creator_1 = require("./get-series-plot-row-creator");
...
function timeScalePointTime(mergedPointData) {
    ...
    return (0, assertions_1.ensureDefined)(result);
}
...
class DataLayer {
    ...
    setSeriesData(series, data) {
        ...
        if (data.length !== 0) {
            ...
            const createPlotRow = (0, get_series_plot_row_creator_1.getSeriesPlotRowCreator)(series.seriesType());
    ...
    updateSeriesData(series, data) {
        ...
        const info = { lastBarUpdatedOrNewBarsAddedToTheRight: (0, get_series_plot_row_creator_1.isSeriesPlotRow)(plotRow) };
        ...
        const insertIndex = (0, algorithms_1.lowerBound)(this._sortedTimePoints, this._horzScaleBehavior.key(newPoint.time), (a, b) => this._horzScaleBehavior.key(a.time) < b);
    ...
    _updateLastSeriesRow(series, plotRow) {
        ...
        if (lastSeriesRow === null || this._horzScaleBehavior.key(plotRow.time) > this._horzScaleBehavior.key(lastSeriesRow.time)) {
            if ((0, get_series_plot_row_creator_1.isSeriesPlotRow)(plotRow)) {
    ...
    _setRowsToSeries(series, seriesRows) {
        if (seriesRows.length !== 0) {
            this._seriesRowsBySeries.set(series, seriesRows.filter(get_series_plot_row_creator_1.isSeriesPlotRow));
