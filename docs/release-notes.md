...
- Added `hoveredSeriesOnTop` option (default: `true`) that renders the currently hovered series above other series in the same pane. (PR [#2076](https://github.com/tradingview/lightweight-charts/pull/2076))
- Added series hit testing for built-in and custom series. Mouse event payloads now include `hoveredItem` and `hoveredTarget`, exposing the hovered series and associated object metadata. Built-in line-like, range-like, and composite renderers all support hit testing, while custom series can opt in via an optional `ICustomSeriesPaneRenderer.hitTest()` hook (a geometry-based fallback is used when the hook isn't provided). (PR [#2076](https://github.com/tradingview/lightweight-charts/pull/2076))
- Added `defaultVisiblePriceScaleId` chart option to control which visible price scale (`'left'` or `'right'`) is preferred as the default when both are available. Defaults to `'right'`. (PR [#2079](https://github.com/tradingview/lightweight-charts/pull/2079))
- Added `tickMarkDensity` option to price scale options, providing control over tick mark label density. A higher value results in more spacing between tick marks and fewer tick marks; a lower value results in less spacing and more tick marks. Defaults to `2.5`. (PR [#2050](https://github.com/tradingview/lightweight-charts/pull/2050))
- Improved dashed and dotted line rendering so that the dash phase remains continuous when a series changes stroke color per data item. Previously, dash patterns would restart at each color boundary, producing visual discontinuities. (PR [#2067](https://github.com/tradingview/lightweight-charts/pull/2067))

**Bug Fixes**

- Fixed line series point markers not being drawn correctly when per-point colors change. (PR [#2064](https://github.com/tradingview/lightweight-charts/pull/2064), fixes [#2017](https://github.com/tradingview/lightweight-charts/issues/2017))
- Fixed a visual bug where a pane's DOM element could persist on screen after removing the last series from that pane via `removeSeries()` when the series contained large values. (PR [#2063](https://github.com/tradingview/lightweight-charts/pull/2063))
- Fixed visual jitter that occurred when charts with `autoSize` enabled were resized within dynamic containers. Resize behaviour triggered by `ResizeObserver` now paints synchronously, avoiding the timing mismatch with container resize. (PR [#2055](https://github.com/tradingview/lightweight-charts/pull/2055), contributed by [@DarrenHaba](https://github.com/DarrenHaba))
- Fixed the description of the `timeToIndex()` method. (PR [#2072](https://github.com/tradingview/lightweight-charts/pull/2072))

**Development**

- Added continuous preview releases from the `master` branch via [pkg.pr.new](https://pkg.pr.new). Each push to `master` now publishes a compact preview package, making it easier to test unreleased changes. See the README for installation instructions. (PR [#2053](https://github.com/tradingview/lightweight-charts/pull/2053))

**Contributors**

We'd like to thank our external contributors for their valuable contributions to this release:

- [@DarrenHaba](https://github.com/DarrenHaba)

[Changes since the last published version](https://github.com/tradingview/lightweight-charts/compare/v5.1.0..v5.2.0).
...
(PR [#1945](https://github.com/tradingview/lightweight-charts/pull/1945))

**Enhancements**

- Added `doNotSnapToHiddenSeriesIndices` option to `CrosshairOptions`. When enabled, the crosshair will snap to the nearest visible series data point instead of snapping to hidden series indices. (PR [#1995](https://github.com/tradingview/lightweight-charts/pull/1995))

**Bug Fixes**

- Fixed price axis label positioning when using plugin views that don't implement the optional `fixedCoordinate()` method. Previously, labels were incorrectly treated as positioned at coordinate 0, causing false overlap detection. (PR [#1993](https://github.com/tradingview/lightweight-charts/pull/1993), fixes [#1986](https://github.com/tradingview/lightweight-charts/issues/1986), contributed by [@tpunt](https://github.com/tpunt))

- Fixed time scale `fitContent` method to properly respect the `rightOffset` option when `rightOffsetPixels` is not set. This addresses a regression introduced in version 5.0.9. (PR [#1989](https://github.com/tradingview/lightweight-charts/pull/1989), fixes [#1988](https://github.com/tradingview/lightweight-charts/issues/1988))
...
[Changes since the last published version](https://github.com/tradingview/lightweight-charts/compare/v5.0.9..v5.1.0).
...
- Added examples demonstrating how to implement indicators (studies) within Lightweight Charts. See the [Analysis Indicators tutorial](https://tradingview.github.io/lightweight-charts/tutorials.md#analysis-indicators) for more information. (PR [#1915](https://github.com/tradingview/lightweight-charts/pull/1915))
...
See [Panes](https://tradingview.github.io/lightweight-charts/docs/panes.md) for more information.
...
See [Chart types](https://tradingview.github.io/lightweight-charts/docs/chart-types.md) for more information about the [Yield Curve Chart](https://tradingview.github.io/lightweight-charts/docs/chart-types.md#yield-curve-chart) and [Options Chart](https://tradingview.github.io/lightweight-charts/docs/chart-types.md#options-chart-price-based) types.
...
- New unified series creation API (see [migration guide](https://tradingview.github.io/lightweight-charts/docs/migrations/from-v4-to-v5.md))
...
See the full migration guide: [Migrating from v4 to v5](https://tradingview.github.io/lightweight-charts/docs/migrations/from-v4-to-v5.md)
...
The [createChartEx](https://tradingview.github.io/lightweight-charts/docs/api/functions/createChartEx) function should be used instead of the usual `createChart` function, and an instance of a class implementing [IHorzScaleBehavior](https://tradingview.github.io/lightweight-charts/docs/api/interfaces/IHorzScaleBehavior) should be provided.
...
Please view the migration guide here: [Migrating from v3 to v4](https://tradingview.github.io/lightweight-charts/docs/migrations/from-v3-to-v4.md).
