...
Workspace mode differs from the standalone mode community authors use: it scopes the package name to `@tradingview/`, depends on the library and the shared plugin utilities through `workspace:*`, marks the plugin as an official catalogue entry, pins the devDependencies to exact versions like the rest of the repository, seeds a `CHANGELOG.md` and `NOTICE`, and adds the catalogue preview page (`src/example/preview.html`, declared as `lwcPlugin.preview`) with the private `@tradingview/lwc-plugin-preview-kit` it is built from — a standalone project gets neither, since the kit is never published. The package has no `LICENSE` of its own: the repository's is copied in by a `prepack` script (npm drops symlinks when packing) and removed again by `postpack`, so every tarball carries it. The folder path the wizard asks for is relative to the repository root regardless of where it was started from.
...
- `pnpm plugins:validate` - checks each package against the package contract: the `@tradingview/lwc-plugin-` name, a valid `version`, a `description`, a licence, public `publishConfig`, a `lightweight-charts` peer range, the `lightweight-charts-plugin` keyword, the `lwcPlugin` block against `scripts/plugins/lwc-plugin-metadata.schema.json`, no leftover scaffold placeholders, and the parts the plugin catalogue renders: a README with a description, `### npm` and `### CDN` tabs under Installation and a usage example, plus the pages declared in `lwcPlugin.demo` and `lwcPlugin.preview`, which both have to exist
- `pnpm plugins:check-compat` - builds the library and the toolkit from the workspace, then runs every plugin's own typecheck and build against them. `--floor` instead installs each plugin's minimum declared peer version from npm into a temporary project and typechecks the plugin sources (not `src/example`) against that
...
- `pnpm plugins:build-demos` - builds each package's `lwcPlugin.demo` and `lwcPlugin.preview` pages into `website/static/plugin-demos/<slug>/` and `website/static/plugin-previews/<slug>/` (both gitignored), with the Vite the package pins. The website's own `start` and `build` scripts run it first, which is how the catalogue frames a real page rather than a repository path
- `pnpm plugins:mock-registry` - packs the plugin packages and serves them as a stand-in npm registry on `:4873`, so the documentation site can build its catalogue before anything is published. Point the site at it with `LWC_CATALOGUE_REGISTRY`; see `website/plugins/plugin-catalogue/README.md` for the full preview workflow
- `pnpm plugins:catalogue` - prints the data the docs site builds its plugin catalogue from: every package that passes `plugins:validate`, with the version, peer range, README and publish date of its release on npm. Unpublished packages are listed by name and get no entry. The docs build runs the same script, so invalid metadata or an unreachable registry fails the docs build, and building the site needs the registry; see `website/plugins/plugin-catalogue/README.md`
...
Each package's `compile.mjs` is a few lines that hand the package's own Vite and dts-bundle-generator to `scripts/plugins/compile-plugin.mjs`, which holds the build shared by every official plugin (the standalone template that community authors get keeps a full copy). A graduated plugin owns both of its pages, and there is only one copy of each: the demo (`src/example/index.html`, the page a developer opens on its own, with the edge cases) and the catalogue preview (`src/example/preview.html`, what the docs site frames next to the README). `pnpm plugins:build-demos` builds both. What is left under `plugin-examples/src/plugins/<name>/example/` is a redirect stub to the built demo, so old gallery links keep working; the gallery's `build:deps` script builds only the toolkit and the plugin packages it still imports itself.
